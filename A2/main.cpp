...
{
  const int nShapes = 2;
  nigmatullin::Shape** shapes = new nigmatullin::Shape*[nShapes];
  shapes[0] = new nigmatullin::Rectangle({ {0.0, 0.0, },  10.0, 10.0, });
  shapes[1] = new nigmatullin::Circle({ 0.0, 0.0, }, 7.0);

  for (size_t i = 0; i < nShapes; ++i)
  {
    std::cout << "This shape is " << shapes[i]->getTypeShape() << "\n";
    std::cout << "Area of this " << shapes[i]->getTypeShape() << " is " << shapes[i]->getArea() << "\n";
    nigmatullin::rectangle_t describedRectangle = shapes[i]->getFrameRect();
